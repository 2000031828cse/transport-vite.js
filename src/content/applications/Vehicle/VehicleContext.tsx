...
interface VehiclesContextType {
  ...
  addFuelLog: (id: number, log: FuelLog) => void;
}
...
export const VehiclesProvider: React.FC<{ children: ReactNode }> = ({
  ...
  const addFuelLog = (id: number, log: FuelLog) => {
    setVehicles((prevVehicles) =>
      prevVehicles.map((vehicle) =>
        vehicle.id === id
          ? { ...vehicle, logs: [...vehicle.logs, log] }
          : vehicle
      )
    );
  };
  ...
  return (
    <VehiclesContext.Provider
      value={{ vehicles, addVehicle, deleteVehicle, updateVehicle, addFuelLog }}
    >
      {children}
    </VehiclesContext.Provider>
  );
